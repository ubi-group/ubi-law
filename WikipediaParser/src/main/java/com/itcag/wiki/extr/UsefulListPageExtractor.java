...
/**
 * Extracts regular pages that belong to the useful lists.
 */
public final class UsefulListPageExtractor {
    
    private final Topic topic;
    
    public UsefulListPageExtractor(Topic topic) {
        this.topic = topic;
    }
    ...
    public final void insertPages() throws Exception {
        ...
        for (Page list : topic.getUsefulLists().values()) {
            HashMap<String, Page> pages = getPages(list.getId());
            filter(pages);
