...
/**
 * Extracts all links from the pages included in the Wikipedia navboxes
 * that were not processed earlier.
 ...
 */
public final class NavboxLinkExtractor {
    ...
    public NavboxLinkExtractor(Topic topic) {
        this.topic = topic;
    }
    ...
    private void collectLinks() throws Exception {
        ...
        for (Page page : topic.getNavboxLinks().values()) {
            if (topic.getPages().containsKey(page.getId())) continue;
//            if (topic.getLinks().containsKey(page.getId())) continue;
...
//                    if (!rejects.containsKey(link.getTail().getTitle())) rejects.put(link.getTail().getTitle(), pageCount);
    ...
    public final void printRejects() {
        System.out.println();
        System.out.println("NavboxLinkExtractor rejects (" + rejects.size() + "):");
